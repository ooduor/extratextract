...
class ExceptionTestCase(base.GenericUtilities, unittest.TestCase):
    """This class contains a bunch of tests to make sure that extratextract
    ...
    def test_unsupported_extension_cli(self):
        ...
        command = "extratextract %(filename)s 2> /dev/null" % locals()
    ...
    def test_unsupported_extension_python(self):
        ...
        from extratextract.exceptions import ExtensionNotSupported
        with self.assertRaises(ExtensionNotSupported):
            extratextract.process(filename)
        os.remove(filename)
    ...
    def test_missing_filename_python(self):
        ...
        os.remove(filename)
        import extratextract
        from extratextract.exceptions import MissingFileError
        with self.assertRaises(MissingFileError):
            extratextract.process(filename)

    def test_shell_parser_run(self):
        """get a useful error message when a dependency is missing"""
        from extratextract.parsers import utils
        from extratextract.parsers import exceptions
